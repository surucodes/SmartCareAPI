...
function AllAppointmentsView({ doctors }: { doctors: Doctor[] }) {
  ...
  return (
    <div className="p-6">
      <h1 className="font-serif text-2xl font-bold text-brand-dark mb-4">All Appointments</h1>
                ...
                <tr key={a.id} className="border-t border-gray-50">
                  ...
                  <td className="px-4 py-2.5">
                    {doctor ? (
                      ...
                      <span className="text-gray-400 italic">Unknown Doctor</span>
                    )}
                  </td>
                  <td className="px-4 py-2.5 text-gray-700">{a.status}</td>
                </tr>
              )
            })}
            {(data ?? []).length === 0 && (
...
export default function AdminDashboardPage() {
  ...
  const allAppointments = useMemo(
    () => Object.values(appointmentsByDoctorId).flat(),
    [appointmentsByDoctorId],
  )

  const selectedAppointment: Appointment | null = useMemo(() => {
    if (!selectedAppointmentId) return null
    return allAppointments.find((a) => a.id === selectedAppointmentId) ?? null
  }, [selectedAppointmentId, allAppointments])

  // When list refetches, re-sync cancellingId
  useEffect(() => {
    if (!cancellingId) return
    const appt = allAppointments.find((a) => a.id === cancellingId)
    if (!appt) {
      setCancellingId(null)
      setSelectedAppointmentId(null)
  ...
  if (selectedNav === 'all') {
    mainContent = (
      <div className="flex-1 overflow-y-auto">
        <AllAppointmentsView doctors={doctors} />
      </div>
    )
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-warm-50">
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        {walkInBanner && (
          <div className="bg-teal-50 border-b border-teal-200 text-teal-800 px-6 py-2 flex items-center justify-between text-sm shrink-0">
      ...
      {!selectedAppointment && !showSearch && !showWalkInModal && (
        <button
          ...
          className="md:hidden fixed bottom-6 right-6 w-14 h-14 bg-teal-800 hover:bg-teal-900 text-white rounded-full shadow-lg flex items-center justify-center z-30 transition-transform active:scale-95"
        >
          <SearchIcon />
        </button>
      )}
